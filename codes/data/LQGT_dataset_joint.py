...
class LQGTDatasetJoint(data.Dataset):
    """
    ...
    def __init__(self, opt):
        super(LQGTDatasetJoint, self).__init__()
        self.opt = opt
        self.data_type = self.opt['data_type']
        self.paths_LQ, self.paths_GT, self.paths_LR = None, None, None
        self.sizes_LQ, self.sizes_GT, self.sizes_LR = None, None, None
        self.LQ_env, self.GT_env, self.LR_env = None, None, None  # environments for lmdb
    ...
    def __getitem__(self, index):
        if self.data_type == 'lmdb' and (self.GT_env is None or self.LQ_env is None):
            self._init_lmdb()
        GT_path, LQ_path, LR_path = None, None, None
        ...
        # get mask image
        # mask_image = cv2.imread("/media/omnisky/ubuntu/zxz/model/mmsr/mask.png")
        ...
        if self.opt['phase'] == 'train':
            ...
            img_LR = img_LR[rnd_h:rnd_h + LQ_size, rnd_w:rnd_w + LQ_size, :]
            rnd_h_GT, rnd_w_GT = int(rnd_h * scale), int(rnd_w * scale)
            img_GT = img_GT[rnd_h_GT:rnd_h_GT + GT_size, rnd_w_GT:rnd_w_GT + GT_size, :]
            # img_mask = mask_image[rnd_h:rnd_h + LQ_size, rnd_w:rnd_w + LQ_size, :]

            # augmentation - flip, rotate
            img_LQ, img_GT, img_LR = util.augment([img_LQ, img_GT, img_LR], self.opt['use_flip'],
                                          self.opt['use_rot'])
        ...
        # 归一化[-1,1]
        # img_GT = trans.Compose([img_GT, trans.Normalize(mean=(0.5, 0.5), std=(0.5, 0.5))])
        # img_LQ = trans.Compose([img_LQ,trans.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))])
        return {'LQ': img_LQ, 'GT': img_GT, 'LR': img_LR, 'LQ_path': LQ_path, 'GT_path': GT_path}
