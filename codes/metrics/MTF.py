...
def superresEdge(edgeProfiles, n=4, returnBins=False):
    ...
    edgeProfiles = [ndimage.shift(profile, -round(shft), cval=float('nan'), order=0) for profile, shft in
                    zip(edgeProfiles, shifts)]
    ...
    for i in range(n):
        ...
        # print 'mean',i, x
        result[
        n - i - 1::n] = x  # The bins are sorted by increasing order of edge position; we want right-most edge to go in first bin.
...
if __name__ == '__main__':
    ...
    if False:
        ...
        plot(lpPerPix_, np.mean([np.abs(MTF(xi)) for xi in x], 0) / x.max(), '.-');
        ylim(0, 1.0)
